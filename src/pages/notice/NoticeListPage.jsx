...
export default function NoticeListPage() {
  ...
  useEffect(() => {
    ...
    const fetchNotices = async () => {
      try {
        ...
        const noticesFromDB = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        // 중요/일반 분리, 정렬, 검색 등 기존 로직 적용
        const important = noticesFromDB.filter((notice) => notice.isImportant);
        const regular = noticesFromDB.filter((notice) => !notice.isImportant);
  ...
  const filterNotices = (noticeList, term) => {
    ...
    return noticeList.filter(
      (notice) =>
        (notice.title && notice.title.toLowerCase().includes(term.toLowerCase())) ||
        (notice.content && notice.content.toLowerCase().includes(term.toLowerCase()))
    );
  };
                ...
                >
                  <div className='px-6 py-4'>
                    ...
                    <div className='flex items-center text-xs text-gray-500'>
                      ...
                      <span className='mx-1'>•</span>
                      <span>
                        {notice.createdAt && notice.createdAt.seconds
